    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="상담 요청 목록">
            <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                {requests.map(req => (
                    <div key={req.id} className={`p-4 rounded-lg ${req.status === 'pending' ? 'bg-dark' : 'bg-dark/50'}`}>
                            ...
                            {req.status === 'pending' && (
                                <div className="flex space-x-2">
                                    <button onClick={() => handleUpdateRequest(req.id, 'confirmed')} className="p-1.5 rounded-full hover:bg-green-500/20" title="확인됨으로 표시">
                                     ...
                                     <button onClick={() => handleUpdateRequest(req.id, 'deleted')} className="p-1.5 rounded-full hover:bg-red-500/20" title="요청 삭제">
